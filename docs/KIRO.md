...
>
> 以上取值均来自本地 `~/.aws/sso/cache/kiro-auth-token.json`:`tokenEndpoint`、`clientId`、`scopes`、`refreshToken`(以及可选的 `accessToken`)。
...
2. 选择凭据类型:
   ...
   - **OAuth · External IdP**:认证方式选择 **External IdP**,填入 `Refresh Token`、`Client ID`、`Token Endpoint`(均必填),以及可选的 `Scopes`。
