export default {
    settings: {
      ...
      openaiCapacityQuarantine: {
        title: 'OpenAI Capacity Temporary Quarantine',
        description: 'Applies reversible, account-level temporary isolation to recognized OpenAI capacity errors. 401, 403, 429, quota, and network errors can never be classified as Capacity.',
        executionNotice: 'The policy is saved immediately. This backend quarantines matching accounts in enforce mode; the default remains disabled. After cooldown, one half-open probe is admitted and a full success recovers the account automatically.',
        mode: 'Mode',
        modeDisabled: 'disabled (off)',
        modeShadow: 'shadow (observe only)',
        modeEnforce: 'enforce (isolate)',
        modeDisabledHint: 'Does not create counters or quarantine accounts.',
        modeShadowHint: 'Matches and records quarantine candidates without changing scheduling.',
        modeEnforceHint: 'Isolates an account after a threshold trip that passes group-floor guards; one half-open probe can recover it after cooldown.',
        errorThreshold: 'Error threshold (count)',
        errorThresholdHint: 'Number of deduplicated Capacity events for one account in the window before a trip. Range 2–20.',
        windowSeconds: 'Counting window (seconds)',
        initialCooldown: 'Initial quarantine (seconds)',
        retripWindow: 'Retrip window (seconds)',
        retripCooldown: 'Retrip quarantine (seconds)',
        maxCooldown: 'Maximum quarantine (seconds)',
        halfOpenTitle: 'Half-open recovery',
        halfOpenHint: 'Only one probe request is admitted after cooldown; long streams renew their lease to prevent concurrent probes.',
        maxRequests: 'Concurrent probes',
        leaseSeconds: 'Probe lease (seconds)',
        renewInterval: 'Lease renewal (seconds)',
        rulesTitle: 'Capacity event match rules',
        rulesHint: 'All conditions in a rule must match; rules are ORed. Only provider_code, provider_type, message and equals/contains_ci are available—no regex or arbitrary scripts.',
        addRule: 'Add rule',
        ruleID: 'Rule ID',
        ruleName: 'Rule name',
        conditionValue: 'Match value',
        addCondition: 'Add condition',
        groupPoliciesTitle: 'Group floor and global-spike protection',
        groupPoliciesHint: 'Only enabled OpenAI groups participate. A trip must leave the configured minimum eligible accounts and comply with the maximum isolation fraction and spike guard.',
        addGroup: 'Add group',
        noOpenAIGroups: 'No active OpenAI groups are available for configuration.',
        groupEnabled: 'Enable for this group',
        group: 'Group',
        minRemaining: 'Minimum remaining accounts',
        maxQuarantinedFraction: 'Maximum quarantined fraction (0–1)',
        globalSpikeAccounts: 'Spike account count',
        globalSpikeWindow: 'Spike window (seconds)',
        matcherTestTitle: 'Rule test (redacted fields only)',
        matcherTestHint: 'This tests the policy currently saved on the server; it sends no upstream request and does not persist the input. Save new or changed rules first.',
        httpStatus: 'HTTP status',
        providerCode: 'provider_code',
        providerType: 'provider_type',
        message: 'Error message (redacted)',
        testMatcher: 'Test match',
        matcherMatched: 'Matched rule: {rule}',
        matcherNotMatched: 'No match (reason: {reason})',
        matcherTestFailed: 'Failed to test Capacity match rule',
        saved: 'OpenAI Capacity temporary quarantine policy saved',
        saveFailed: 'Failed to save OpenAI Capacity temporary quarantine policy'
      },
