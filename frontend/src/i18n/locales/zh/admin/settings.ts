export default {
    settings: {
      ...
      openaiCapacityQuarantine: {
        title: 'OpenAI Capacity 临时摘除',
        description: '针对已识别的 OpenAI 容量错误，按账号实施可回滚的临时隔离策略。401、403、429、配额和网络错误不会被识别为 Capacity。',
        executionNotice: '策略会立即保存。当前后端在 enforce 模式会实际摘除命中账号；默认 disabled。首次冷却结束后只放行一个 half-open 探测，完整成功会自动恢复。',
        mode: '运行模式',
        modeDisabled: 'disabled（关闭）',
        modeShadow: 'shadow（只观测）',
        modeEnforce: 'enforce（执行隔离）',
        modeDisabledHint: '不新增计数、不隔离账号。',
        modeShadowHint: '匹配事件并记录候选摘除，但不会影响调度。',
        modeEnforceHint: '阈值命中并通过分组保底保护后隔离账号；冷却后仅一个 half-open 探测可自动恢复。',
        errorThreshold: '触发阈值（次）',
        errorThresholdHint: '同一账号在窗口内命中多少个去重后的 Capacity 事件才触发。范围 2–20。',
        windowSeconds: '统计窗口（秒）',
        initialCooldown: '首次隔离（秒）',
        retripWindow: '再次触发窗口（秒）',
        retripCooldown: '再次隔离（秒）',
        maxCooldown: '最大隔离（秒）',
        halfOpenTitle: 'Half-open 自动恢复',
        halfOpenHint: '冷却到期仅允许一个探测请求；长流会按租约续期，避免并发探测。',
        maxRequests: '并发探测数',
        leaseSeconds: '探测租约（秒）',
        renewInterval: '租约续期（秒）',
        rulesTitle: 'Capacity 事件识别规则',
        rulesHint: '每条规则的全部条件都必须命中；规则之间为“或”。仅支持 provider_code、provider_type、message 与 equals/contains_ci，不能使用正则或任意脚本。',
        addRule: '添加规则',
        ruleID: '规则 ID',
        ruleName: '规则名称',
        conditionValue: '匹配值',
        addCondition: '添加条件',
        groupPoliciesTitle: '分组保底与全局尖峰保护',
        groupPoliciesHint: '仅启用的 OpenAI 分组会参与策略；隔离前需保留最低可用账号数量，并受最大隔离比例和尖峰保护限制。',
        addGroup: '添加分组',
        noOpenAIGroups: '没有可配置的活跃 OpenAI 分组。',
        groupEnabled: '此分组启用',
        group: '分组',
        minRemaining: '保底账号数',
        maxQuarantinedFraction: '最大隔离比例（0–1）',
        globalSpikeAccounts: '尖峰账号数',
        globalSpikeWindow: '尖峰窗口（秒）',
        matcherTestTitle: '规则测试（仅脱敏字段）',
        matcherTestHint: '测试的是服务器当前已保存的规则；不会发送上游请求，也不会保存输入内容。请先保存新增或修改后的规则。',
        httpStatus: 'HTTP 状态码',
        providerCode: 'provider_code',
        providerType: 'provider_type',
        message: '错误消息（脱敏）',
        testMatcher: '测试匹配',
        matcherMatched: '命中规则：{rule}',
        matcherNotMatched: '未命中（原因：{reason}）',
        matcherTestFailed: '测试 Capacity 识别规则失败',
        saved: 'OpenAI Capacity 临时摘除策略已保存',
        saveFailed: '保存 OpenAI Capacity 临时摘除策略失败'
      },
