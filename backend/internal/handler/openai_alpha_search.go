...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if err == nil {
			h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestedModel), true, nil)
			if result != nil {
		...
		if !errors.As(err, &failoverErr) {
			h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestedModel), false, nil)
