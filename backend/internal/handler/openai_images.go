...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestModel), !retryableServerError, nil)
				...
				if errors.As(err, &failoverErr) {
					h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestModel), false, nil)
		...
		if result != nil {
			...
			h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestModel), true, result.FirstTokenMs)
		} else {
			h.reportOpenAIAccountScheduleResult(c, account.ID, account.GetMappedModel(requestModel), true, nil)
		}
